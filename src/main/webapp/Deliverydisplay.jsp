            ...
            <main class="p-6">
                <div class="bg-white p-4 rounded-md shadow-md">
                    <div class="flex justify-between mb-4">
                        <h3 class="text-lg font-semibold">Delivery Records</h3>
                        <button class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 flex items-center space-x-2" onclick="location.href='DeliveryInsert.jsp'">
                    ...
                    <table class="w-full border-collapse border border-gray-200">
                       ...
                       <c:forEach var="delivery"  items="${allDelivery}">
                        <tr>
                            ...
                            <td class="border border-gray-300 px-4 py-2">${delivery.nic}</td>
                            <td class="border border-gray-300 px-4 py-2">${delivery.vehicleType}</td>
                            <td class="border border-gray-300 px-4 py-2">${delivery.licenNo}</td>
                            <td class="border border-gray-300 px-4 py-2">${delivery.phone}</td>
                            
                            <td class="border border-gray-300 px-4 py-2 space-x-2">
                            <div class="flex items-center space-x-2">
    						<a href="DeliveryUpdate.jsp?id=${delivery.id}&name=${delivery.name}&nic=${delivery.nic}&vehicleType=${delivery.vehicleType}&licenNo=${delivery.licenNo}&phone=${delivery.phone}">
        						<button class="bg-yellow-500 text-white px-3 py-1 rounded-md hover:bg-yellow-600">
            						<i data-lucide="edit"></i>
        						</button>
    						</a>
    						<form action="DeliveryDeleteServlet" method="POST">
